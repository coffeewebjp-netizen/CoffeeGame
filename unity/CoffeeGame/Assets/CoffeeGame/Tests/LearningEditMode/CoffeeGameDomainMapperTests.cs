...
namespace CoffeeGame.Integration.Tests
{
    public sealed class CoffeeGameDomainMapperTests
    {
        ...
        [TestCase("")]
        [TestCase("   ")]
        public void CompletedIncorrectResultWithoutGrantMapsAsMistake(string wireGrantId)
        {
            var response = CreateCompletedResponse();
            response.result.judgment.isCorrect = false;
            response.result.learning.state = CoffeeGameContractV1.MistakeLearningState;
            response.result.rewardEligibility.eligible = false;
            response.result.rewardEligibility.grantId = wireGrantId;

            var outcome = CoffeeGameDomainMapper.ToAuthoritativeOutcome(response);

            Assert.That(outcome.IsCorrect, Is.False);
            Assert.That(outcome.LearningMutationApplied, Is.True);
            Assert.That(outcome.RewardEligible, Is.False);
            Assert.That(outcome.GrantId, Is.Null);
        }

        [Test]
        public void CompletedIncorrectResultStillRejectsRealGrantId()
        {
            var response = CreateCompletedResponse();
            response.result.judgment.isCorrect = false;
            response.result.learning.state = CoffeeGameContractV1.MistakeLearningState;
            response.result.rewardEligibility.eligible = false;
            response.result.rewardEligibility.grantId = "gr_invalid_for_mistake";
