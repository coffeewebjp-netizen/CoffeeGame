...
$backup = Join-Path $repo '.task-local-backup\ORC-20260906-005-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260906-005' -or $manifest.inputId -ne 'IN01' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-acrobatics-v11 player. Current character selection and save progress stay unchanged."
  exit
}
