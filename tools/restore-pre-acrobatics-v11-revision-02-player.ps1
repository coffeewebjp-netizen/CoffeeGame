...
$backup = Join-Path $repo '.task-local-backup\ORC-20260906-005-revision-02-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260906-005' -or $manifest.inputId -ne 'IN03' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-acrobatics-v11-revision-02 player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-acrobatics-v11') $backup
...
Write-Output 'Pre-acrobatics-v11 player restored. Character selection was not changed. Save progress and unrelated settings were preserved.'
