...
$backup = Join-Path $repo '.task-local-backup\ORC-20260906-003-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260906-003' -or $manifest.inputId -ne 'IN01' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-defense-v10 player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-defense-v10') $backup
...
Write-Output 'Pre-defense-v10 player restored. Character selection was not changed. Save progress and unrelated settings were preserved.'
