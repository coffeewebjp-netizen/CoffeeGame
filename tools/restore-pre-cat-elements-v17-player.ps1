...
$backup = Join-Path $repo '.task-local-backup\ORC-20260909-001-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260909-001' -or $manifest.inputId -ne 'IN01' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-cat-elements-v17 player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-cat-elements-v17') $backup
