...
$backup = Join-Path $repo '.task-local-backup\ORC-20260905-001-WP19-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260905-001' -or $manifest.inputId -ne 'IN15' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-action player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-action-v5') $backup
...
Write-Output 'Pre-action player restored. Character selection was not changed. Save progress and unrelated settings were preserved.'
