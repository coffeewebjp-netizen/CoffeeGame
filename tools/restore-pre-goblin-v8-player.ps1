...
$backup = Join-Path $repo '.task-local-backup\ORC-20260905-001-WP22-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260905-001' -or $manifest.inputId -ne 'IN18' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-goblin player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-goblin-v8') $backup
...
Write-Output 'Pre-goblin player restored. Character selection was not changed. Save progress and unrelated settings were preserved.'
