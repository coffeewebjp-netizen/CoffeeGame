...
$backup = Join-Path $repo '.task-local-backup\ORC-20260909-001-in03-normal-player'
...
if ($manifest.taskId -ne 'ORC-20260909-001' -or $manifest.inputId -ne 'IN03' -or $manifest.normal -ne $normal) { throw 'Unexpected backup identity or location.' }
...
if (-not $Restore) {
  Write-Output "Verified $($manifest.oldFiles.Count) old files and $($manifest.newFiles.Count) installed files. No changes made. Use -Restore to restore the pre-cat-motion-v19 player. Current character selection and save progress stay unchanged."
  exit
}
...
$retained = Within (Join-Path $backup 'rollback-retained-cat-motion-v19') $backup
...
Write-Output 'Pre-cat-motion-v19 player restored. Character selection was not changed. Save progress and unrelated settings were preserved.'
