...
2026-09-06 ORC-20260906-002 / Party V9：親密度100での猫少女加入を既存記録と接続し、別々の能力値・HP/MP/ST、AI共闘、T/RBでの操作切替、通常魔法弾・大魔法・10秒の選択的時間停止を実装。片方の戦闘不能は交代して続行、全員戦闘不能で敗北。10分でHP1%に復活、休息中は約15分でHP/MPが全回復し、ポーズ・終了中も実時間で進む。プロフィールv3とv1/v2移行、未知版上書き防止、旧版向け復元を実装した。主人公ボイス4本、戦闘BGM「疾走する精霊の森」v2、動作効果音9本も組み込んだ。245件の関連テスト、専用素材検証、Windowsビルド、実行時15項目と4画面の描画確認を通過。Google Drive実保存先の安全な置換を専用仮ファイルで検証し、現在のセーブは不変。作業前1,800ファイルのうち今回範囲外の変更は0。旧通常版348ファイルと新297ファイルの退避・準備は完了しているが、通常Steam起動先の置換は自動承認レビューが明示Owner承認を要求したため未実施。公開pushも未実施。[操作・復元・検証](cat-companion-v9.md)、[モデルと動作の出典](../art/3d/trials/meshy-rival/README.md)。
